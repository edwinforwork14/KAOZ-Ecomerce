...
export default function Header({ activeTab, setActiveTab }: HeaderProps) {
  ...
  const handleUserAction = () => {
    if (user) {
      // Si ya hay usuario, podríamos ir al dashboard o cerrar sesión
      // Por ahora, pondremos un pequeño menú o simplemente logout
      ...
      <div className="bg-kaosBlack text-white text-[10px] py-2 px-4 md:px-10 flex justify-between items-center uppercase tracking-widest z-[60] relative">
        ...
        <div className="flex gap-6">
          ...
          >
            ...
            {user ? `${user.user_metadata?.first_name || 'MI CUENTA'}` : 'MI CUENTA'}
          </button>
        </div>
      </div>
